...
An empirical panel OLS and WLS investigation using monthly Census Bureau trade data and the Center for Global Development (CGDev) tariff tracker.
...
This project estimates the **import demand elasticity** with respect to the added effective tariff rate: how much did a onepercentage-point increase in the tariff rate reduce US monthly imports from a given country? The answer matters for several reasons:
...
This analysis builds an econometrically valid panel framework for estimating that elasticity. US monthly import data for each partner country was only available until January 2026, and modeling this dataset does not yet show a discernible import response among major trading partners. The model, however, is well-positioned to produce precise estimates as 2026 import data becomes available, particularly once tariff variation following the SCOTUS's February 2026 IEEPA ruling can be tested.
...
$$\beta_1 \approx \frac{\Delta \log(\text{Imports})}{\Delta \text{TariffRate}} \approx \text{percentage change in monthly imports per 1 percentage point increase in tariff rate}$$
...
After two-way demeaning, the within-country standard deviation of the tariff rate is **2.68 percentage points** (without China), confirming that meaningful identifying variation exists in the panel. However, as the robustness results below show, this variation is distributed unevenly: it is concentrated among smaller trading partners with more volatile trade flows, while large partners experienced a more uniform tariff path over the sample period.
...
The full-sample baseline yields β₁ = −0.0273 (p < 0.001) in the preferred no-lag specification, statistically significant and consistent across lag structures. However, the within-R² of just 1% is a warning sign: tariff variation is explaining very little of the total variance in import growth, even where the coefficient is significant. The robustness checks below clarify why.
...
Specification                           β₁      t-stat   p-value    N    R²(within)
-------------------------------------------------------------------------------------
No Lag   | Without China            +0.0005      +0.10    0.9179   1788    -0.0004
1-Mo Lag | Without China            +0.0029      +0.72    0.4736   1608     0.0007
No Lag   | With China               +0.0018*     +2.49    0.0130   1798     0.0041
1-Mo Lag | With China               -0.0009.     -1.72    0.0857   1617     0.0007
...
Specification                           β₁      t-stat   p-value    N    R²(within)
-------------------------------------------------------------------------------------
No Lag   | Without China            -0.0022      -0.51    0.6114    491     0.0045
1-Mo Lag | Without China            -0.0024      -0.52    0.6063    442     0.0013
No Lag   | With China               +0.0003      +0.22    0.8266    501    -0.0004
1-Mo Lag | With China               -0.0015      -1.55    0.1218    451     0.0013
```

Both robustness checks produce coefficients indistinguishable from zero for large trading partners. The WLS without-China estimate collapses from −0.0273 to essentially zero (+0.0005, p = 0.92), and the Top-50 without-China estimate is similarly flat (−0.0022, p = 0.61). This pattern across two independent approaches indicates that **the baseline result is driven by small and volatile trading partners** - countries where a small absolute change in imports translates to a large percentage swing - rather than by the economically significant bilateral relationships that dominate US trade.

**The +0.0018\* anomaly (WLS, No Lag, With China)** warrants specific explanation. This statistically significant *positive* coefficient - implying more imports when tariffs rise - is an artifact of the April front-running problem interacting with China's dominant weight in the WLS sample. China's within-country variation pairs April (tariff: 111.8%, imports: $25.4B, inflated by pre-tariff stockpiling) with subsequent months (lower tariffs, lower imports), creating a spurious positive correlation between tariffs and import levels when China is heavily weighted. The 1-Mo Lag WLS with China is negative and marginally significant (−0.0009, p = 0.086), consistent with this interpretation: the lag partially decouples the front-running April baseline from the tariff value, partially correcting the sign.

**Effect of including China:** In the baseline, adding China reduces the estimated coefficient from −0.0273 to −0.0175, reflecting that China's enormous tariff swing (from 111.8% in April to ~30% subsequently) is accompanied by a smaller-than-expected import decline, consistent with front-running inflating the April baseline. The without-China result is more representative of the universal tariff effect on the general population of trading partners.
...
**Pre-tariff front-running.** April 2025 import data likely reflects elevated pre-tariff stockpiling, inflating the April baseline. This creates a mechanical negative-looking relationship for the no-lag spec (high April imports followed by lower imports in subsequent months) that may be partially confounded with the tariff effect. The consistency of the no-lag and 1-Mo Lag baseline results somewhat mitigates this concern, but it cannot be fully resolved without pre-April data extending further back.

**February 2026 SCOTUS escalation.** The panel currently ends January 2026, before the tariff increases following the Supreme Court ruling on IEEPA authority in February 2026. This is the most important near-term extension: that ruling introduced renewed tariff variation across a different set of countries and product categories, providing genuinely new cross-country identifying variation that the current panel lacks. Adding data through mid-2026 should substantially sharpen the elasticity estimate for large trading partners.
